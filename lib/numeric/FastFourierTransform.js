var sinTable = [];
var cosTable = [];
var _TypedArray = Float64Array || Float32Array;
 ...
 */
function _getReverseTable(length) {
    if (reverseTables[length]) return reverseTables[length];
    var prevTable = _getReverseTable(length >> 1);
    var i, half = length >> 1;
...
function _fft_r2(list, n, offset, step) {
    var i, m, iteration, k, omreal, omimag, oreal, oimag, id1, id2,
        tr, ti, tmpReal,
        rev, a,
        reverseTable = _getReverseTable(n),
        offset2 = offset << 1,
        step2 = step << 1;
...
function _conv(list1, list2, n) {
    ...
    var minn = n * 2 - 1,
        ...
        length2 = length << 2,
        i, r1, i1, r2, i2;
    _fft_r2(list1, length, 0, 1);
    _fft_r2(list2, length, 0, 1);
...
function _fft_bluestein(list, n, offset, step) {
    var nextPo2 = _nextpo2(n),
        length2 = n * 2,
        an = new _TypedArray(nextPo2 * 4),
        bn = new _TypedArray(nextPo2 * 4),
        ro, io, cos, sin,
        offset2 = offset * 2,
        ang = Math.PI / n,
        a;
    for (var i = 0, j = 0; i < length2; i += 2, j++) {
        ro = list[i * step + offset2];
        io = list[i * step + offset2 + 1];
        a = j * j * ang;
        cos = Math.cos(a);
        sin = -Math.sin(a);
        an[i] = cos * ro - sin * io;
