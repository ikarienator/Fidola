class FastFourierTransform {
  ...
  static sinTable:{[name:string]:number;} = {};
  static cosTable:{[name:string]:number;} = {};
  static reverseTable:number[][] = [];

  sinTable:{[name:string]:number;};
  cosTable:{[name:string]:number;};
  reverseTable:number[];
...
export function fft(list, length) {
  var i,
      len = list.length,
      n = length || len,
...
export function ifft(list, length) {
  var i,
      len = list.length,
      n = length || len,
      expn = 1 << Math.ceil(Math.log(n) / Math.log(2));
